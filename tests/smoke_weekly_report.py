...
# 「04. 주차별 추이」 검사는 여기 있었다. 그 페이지를 접고 「02」 ②가 흡수하면서
# `test_funnel_page.py`로 옮겼다 — 차트 개수·전년비 유무·기간 개수·값|증감 배치를
# 거기서 본다(`t_trend_block_draws_charts_and_a_yoy_table` 외).
...
def check_push_year_picker():
    ...
    try:
        ...
        r.set_value("05. 앱푸시 동의 현황"); at.run()
        if at.exception:
