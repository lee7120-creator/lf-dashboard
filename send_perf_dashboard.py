...
# ── 브랜드 자동 분류 (문구 기반) ────────────────────────────────────────
# 실적 엑셀의 '브랜드' 칸은 담당자가 손으로 적어서 캠페인명·행사명이 섞여 들어온다
# ('윈터세일', 'L+DAY 쇼핑핫타임 1차', '주문서이탈'…). 그대로 차원으로 쓰면 브랜드별
# 비교가 안 되므로, 제목·내용·브랜드칸을 같이 훑어 실제 브랜드를 뽑아낸다.
#
# 표기 정규화 후 매칭한다 — 공백·중점·하이픈을 지우고 대문자로 올린 문자열에서 찾으므로
# 사전에는 '질스튜어트'만 넣어도 '질 스튜어트'·'질·스튜어트'가 같이 잡힌다.
#
# 브랜드 사전은 영업별 운영브랜드 시트에서 만든 data/brand_map.csv 가 정본이다
# (brand=base 브랜드명, org=영업). 갱신은 tools/build_brand_map.py 로 다시 생성한다.
#
# ▼ 아래 ALIAS 는 시트에 없는 표기만 손으로 보탠다 — 영문 표기·사내 약어 등.
#   값은 반드시 brand_map.csv 의 brand 와 정확히 같아야 한다.
BRAND_ALIAS = {
    "HAZZYS": "헤지스", "해지스": "헤지스",
    "DAKS": "닥스", "DAKSLADIES": "닥스", "DAKSLONDON": "닥스런던",
    "MAESTRO": "마에스트로",
    "TNGT": "티엔지티",
    "JILLSTUART": "질스튜어트", "JILLBYJILLSTUART": "질 바이 질스튜어트",
    "ILCORSO": "일꼬르소",
    "ALLEGRI": "알레그리",
    "VANESSABRUNO": "바네사브루노",
    "ISABELMARANT": "이자벨마랑",
...
# 문구에 브랜드가 안 잡히는데 이 키워드가 걸리면 '전관행사'로 묶는다.
PROMO_KW = [
    "L+DAY", "L%2BDAY", "LDAY", "엘프", "엘플",
    "쇼핑핫타임", "핫타임", "나이트마켓", "룰렛", "럭키먼데이", "뷰티데이",
    "블랙프라이데이", "블프", "창립제", "정기세일", "그랜드세일",
    "윈터세일", "서머세일", "썸머세일", "위켄드세일", "주말특가", "패밀리세일",
    "단독특가전", "브랜드데이", "슈퍼세일", "메가세일", "전관행사",
]

# 브랜드가 아니라 '발송 트리거·유형'을 적어 둔 값. 브랜드칸에서만 본다 — 문구에까지
# 적용하면 '무료배송'·'장바구니에 담아두신'처럼 흔한 표현이 죄다 트리거로 잡힌다.
TRIGGER_KW = [
    "주문서이탈", "장바구니", "재입고알림", "이탈", "리마인드", "남은모수",
    "휴면", "생일", "구매감사", "리뷰요청", "찜", "위시",
]

_BR_NORM_RE = re.compile(r"[\s·\-_/&,\.\(\)\[\]']+")


def _br_norm(s):
    """브랜드 매칭용 정규화 — 공백·구분자 제거 + 대문자.

    (모듈 전역 헬퍼 `_s`보다 앞에서 정의되므로 NaN 처리를 여기서 직접 한다.)
    """
...
BRAND_MAP_CSV = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                             "data", "brand_map.csv")

# 짧은 이름을 문구 안에서 부분 일치로 찾으면 오탐이 쏟아진다 — 입점 브랜드엔 '객'·'갭'·
# '고요'처럼 일반 한국어에 그대로 박히는 이름이 많다. 그래서 길이로 나눈다.
#   · 3글자 이상        → 전부 부분 일치 허용
#   · 2글자 자사(영업1·2) → 부분 일치 허용 (닥스·빈스·리복·빠투 같은 주력 브랜드)
#   · 그 외 2글자 이하   → 브랜드칸이 정확히 그 값일 때만 인정
_BRAND_MIN_LEN = 3
...
def _load_brand_map(path=BRAND_MAP_CSV):
    """data/brand_map.csv → (정규화명→(브랜드, 영업)) 사전 2개.

    반환: (long_map, exact_map)
      long_map  — 문구 안에서 부분 일치로 찾을 이름
      exact_map — 브랜드칸이 정확히 일치할 때만 쓸 이름
    """
    long_map, exact_map = {}, {}
    try:
        ...
        return long_map, exact_map                        # 파일이 없어도 앱은 떠야 한다
    for r in rows:
        b = (r.get("brand") or "").strip()
        if not b:
            continue
        org = (r.get("org") or "").strip() or ORG_UNKNOWN
        n = _br_norm(b)
        if not n:
            continue
        partial = (len(n) >= _BRAND_MIN_LEN or (len(n) == 2 and org in _OWN_ORGS))
        if b in BRAND_STOP or n in BRAND_STOP:
            partial = False
        (long_map if partial else exact_map).setdefault(n, (b, org))
...
KIND_BRAND = "브랜드"
KIND_PROMO = BRAND_PROMO
KIND_TRIGGER = BRAND_TRIGGER
KIND_PARTNER = "제휴몰"
KIND_CATEGORY = "카테고리 묶음"
KIND_ETC = "기타"

# 브랜드가 아니라 제휴처·외부몰을 적어 둔 값
PARTNER_KW = [
    "한국금거래소", "아모레", "올리브영", "이마트", "롯데", "신세계", "현대백화점",
    "제휴", "입점몰", "위메프", "쿠팡", "네이버", "카카오",
]
PARTNER_RE = re.compile("|".join(re.escape(_br_norm(w)) for w in PARTNER_KW))


def _partner_label(brand_raw):
    """제휴몰은 적어 둔 이름을 그대로 살린다 — 어디 제휴인지가 정보다."""
    s = " ".join(str(brand_raw or "").split())
    return s[:20] if s else KIND_PARTNER


def brand_from_copy(title="", body="", brand_raw="", cat=""):
    """제목·내용·브랜드칸 → (브랜드, 영업, 구분).

    담당자가 적은 브랜드칸을 먼저 본다 — 'L:ABLE - DAKS LADIES'·'헤지스남성 아울렛'처럼
    행사명이 섞여 있어도 브랜드가 같이 적혀 있는 경우가 많아서다. 거기서 못 찾으면
    제목 → 내용 순으로 문구를 훑는다. 한 문자열에 여러 브랜드가 있으면 가장 긴 이름을
    택한다('질스튜어트 뉴욕'이 '질스튜어트'보다 우선).

    브랜드가 끝내 안 잡히면 그냥 미분류로 버리지 않고 성격별로 묶는다 —
    전관행사 → 제휴몰 → 트리거발송 → 카테고리 묶음('골프 외') → 기타 순.
    """
    nb, nt, nbody = _br_norm(brand_raw), _br_norm(title), _br_norm(body)
    ...
    full = nb + nt + nbody
    if PROMO_RE.search(full):
        return (BRAND_PROMO, ORG_UNKNOWN, KIND_PROMO)
    if PARTNER_RE.search(full):
        return (_partner_label(brand_raw), ORG_UNKNOWN, KIND_PARTNER)
    if nb and TRIGGER_RE.search(nb):                      # 트리거는 브랜드칸에서만 본다
        return (BRAND_TRIGGER, ORG_UNKNOWN, KIND_TRIGGER)
    c = " ".join(str(cat or "").split())
    if c and c not in ("통합", "-", "nan"):
        # 브랜드는 못 집었지만 어느 카테고리 발송인지는 안다 — 그 단위로 묶어 준다
        return (f"{c} 외", ORG_UNKNOWN, KIND_CATEGORY)
    return (BRAND_UNKNOWN, ORG_UNKNOWN, KIND_ETC)
...
def add_tags(df):
    ...
    # 브랜드 자동 분류 — 담당자가 적은 brand 칸이 캠페인명으로 오염돼 있어, 문구와 함께
    # 훑어 영업별 운영브랜드 시트 기준으로 다시 태깅한다. 영업(org)도 같이 붙인다.
    brands = df["brand"].tolist() if "brand" in df else [""] * len(df)
    cats = df["cat"].tolist() if "cat" in df else [""] * len(df)
    _bo = [brand_from_copy(t, b, br, c)
           for t, b, br, c in zip(titles, bodies, brands, cats)]
    tdf["brand2"] = [x[0] for x in _bo]
    tdf["sales_org"] = [x[1] for x in _bo]
    tdf["brand_kind"] = [x[2] for x in _bo]
...
# ── 사이드바 메뉴 ────────────────────────────────────────────────────────
# **여기 한곳**이 페이지 목록이자 순서다. 번호는 표시용이라 안내 문구는
# `menu_ref()`로 여기서 만들어 쓴다 — 문구에 번호를 박으면 재정렬 때 썩는다.
#
# 묶는 기준은 **답하는 질문**이다. 예전엔 13개 그룹이었고 그중 다섯이 한 탭짜리라,
# 사이드바는 긴데 정작 「무엇부터 볼까」가 안 보였다. 지금은 10개고 한 탭짜리는
# 둘뿐이다(주간보고·데이터).
#   · 「종합 요약」에 전체 효율·추이를 합쳤다 — 둘 다 '전반이 어떤가'다.
#   · 「캠페인 리더보드」·「기획전 비교」를 「성과 진단」으로 넣었다 — 축만 다른 랭킹이다.
#   · 「앱푸시 동의 현황」·「회원UV·거래액」을 「사이트·앱 지표」로 묶었다 —
#     둘 다 CRM 발송 실적이 아니라 그 바깥의 모수·환경이다.
#   · 「액션」은 뭘 하는 화면인지 이름만으로 안 읽혀 「다음 발송 준비」로 바꿨다.
# 하위탭 이름은 아래 페이지 분기(`in page`)의 매칭 키를 그대로 품어야 한다.
CAMPAIGN_GROUPS = {
    "1. 주간보고":         ["주간보고"],
    "2. 종합 요약":        ["종합 요약", "전체 효율·추이"],
    "3. 문구 분석":        ["문구 속성별 성과", "키워드·이모지 성과", "소구 추세·마모"],
    "4. 성과 진단":        ["캠페인 리더보드", "전환·AOV 진단", "발송유형·브랜드",
                           "BPU·우선순위", "기획전 비교"],
    "5. 타이밍·카테고리":  ["카테고리·시간대", "타이밍·발송슬롯", "전년 동요일 비교"],
    "6. 발송량·피로도":    ["피로도 시계열", "발송 빈도·한계수익", "발송량 최적 구간",
                           "요일 패턴", "유입 퍼널"],
    "7. 개선 효과 검증":   ["문구 개선 효과", "잔여모수 추가 효과", "컨틴 구좌 효율",
                           "발송 감축 효과"],
    "8. 다음 발송 준비":   ["발송 플레이북", "AI 처방·카피"],
    "9. 사이트·앱 지표":   ["회원UV·거래액", "앱푸시 동의 현황"],
    "10. 데이터·다운로드": ["데이터·다운로드"],
}

# 전사 MTD 파일이 있어야 열리는 하위탭 — **한 그룹에 모아 둔다**. 분기 조건과
# 사이드바 안내가 같은 목록을 봐야 한 쪽만 고쳐져 갈리지 않는다.
MTD_TABS = ("피로도 시계열", "발송 빈도·한계수익", "발송량 최적 구간",
            "요일 패턴", "유입 퍼널")


def menu_ref(tab):
    """안내 문구에 넣을 「N. 그룹 › 하위탭」 — 메뉴에서 만들어 번호가 안 썩는다.

    예전엔 안내 문구가 번호까지 박은 이름을 그대로 들고 있어서, 페이지를 재정렬하면
    **없는 번호로 사람을 안내**했다. 화면은 멀쩡히 떠서 눈으로는 안 잡힌다."""
    for grp, subs in CAMPAIGN_GROUPS.items():
        if tab in subs:
            return grp if len(subs) == 1 else f"{grp} › {tab}"
    return tab
...
def main():
    ...
    .psx-do{font-size:26px;font-weight:700;color:var(--psx-c,#1e293b);line-height:1.25;margin-bottom:11px}
    .psx-li{font-size:14.5px;color:#475569;line-height:1.7;padding-left:13px;position:relative}
    .psx-li:before{content:"·";position:absolute;left:2px;color:#94a3b8}
    .psx-kpi{margin-top:11px;padding-top:9px;border-top:1px dashed #e2e8f0;font-size:13.5px;color:#64748b}
    .psx-kpi b{color:#334155;font-weight:600}
    /* 흐름도 */
    .psx-flow{display:flex;flex-wrap:wrap;align-items:stretch;gap:7px;margin:10px 0}
    /* max-width가 없으면 줄바꿈된 마지막 노드가 남은 폭을 통째로 먹어 흐름이 깨진다 */
    .psx-node{flex:1 1 128px;max-width:210px;background:#fff;border:1px solid #e2e8f0;
      border-radius:8px;padding:12px 12px;text-align:center;min-width:124px}
    .psx-node.done{background:#f0f9f4;border-color:#bbe5cd}
    .psx-node .n1{font-size:14.5px;font-weight:600;color:#1e293b;line-height:1.4}
    .psx-node .n2{font-size:12.5px;color:#94a3b8;margin-top:3px}
    .psx-arw{display:flex;flex-direction:column;align-items:center;justify-content:center;
      font-size:13px;color:#94a3b8;padding:0 2px;min-width:60px}
    .psx-arw b{color:#2E68B0;font-size:13px;font-weight:600;white-space:nowrap}
    /* KPI 계층 */
    .psx-kgrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(212px,1fr));gap:9px;margin:8px 0}
    .psx-kcol{background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:15px 17px}
    .psx-kh{font-size:15px;font-weight:700;color:#1e293b;margin-bottom:9px;
      display:flex;align-items:center;gap:7px}
    .psx-kn{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;
      border-radius:50%;background:#1e293b;color:#fff;font-size:12px;font-weight:700;flex-shrink:0}
    .psx-kli{font-size:14px;color:#475569;line-height:1.75}
    .psx-hero{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:9px;margin:10px 0 0}
    .psx-hcard{background:#0f172a;color:#e2e8f0;border-radius:8px;padding:15px 18px}
    .psx-hcard .hl{font-size:12.5px;color:#7dd3fc;letter-spacing:.09em;font-weight:700}
    .psx-hcard .hv{font-size:16.5px;font-weight:600;margin-top:5px;line-height:1.45;color:#fff}
    /* 로드맵 */
    .psx-road{display:grid;grid-template-columns:repeat(auto-fit,minmax(248px,1fr));gap:9px;margin:8px 0}
    .psx-rcard{background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:15px 17px;
      border-left:3px solid var(--psx-c,#94a3b8)}
    .psx-rh{font-size:15.5px;font-weight:700;color:#1e293b}
    .psx-rw{font-size:12.5px;color:#94a3b8;margin-bottom:9px}
    ...
    # ── 표 렌더 + 엑셀 내려받기 ──────────────────────────────────────────
    # st.dataframe 기본 툴바의 'Download as CSV'는 서식이 다 날아간다. 모든 표에
    # 같은 자리에서 검은 헤더 스타일 xlsx를 받을 수 있게 공용 래퍼로 감싼다.
    ...
    if len(_subs) > 1:
        ...
        page = _subs[0]
    # 그룹 이름이 아니라 **하위탭 목록**으로 가른다 — 이름을 바꿔도 안내가 안 끊긴다.
    _mtd_here = [x for x in _subs if x in MTD_TABS]
    if _mtd_here and mtd_data is None:
        st.sidebar.info("**" + " · ".join(_mtd_here)
                        + "** 은 전사 MTD 파일을 올리면 볼 수 있어요.")
    ...
    if _n_future_rows:
        st.sidebar.warning(f"⚠️ 미래 날짜로 기록된 {_n_future_rows}건을 분석에서 제외했어요 "
                           ...
                           f"상세는 「{menu_ref('데이터·다운로드')}」에서 볼 수 있어요.")
    ...
    if len(_rw):
        ...
        if len(_lw) >= 5 and _lw["matched"].mean() < 0.7:
            st.sidebar.warning(f"⚠️ 최신 주 매칭률 {_lw['matched'].mean()*100:.0f}% — "
                               ...
                               f"상세는 「{menu_ref('데이터·다운로드')}」의 매칭 품질에서 볼 수 있어요.")
    if af_rejected_msgs:
    ...
    def render_messages(d, mcol, key, n=200, show_hour=False, show_tags=False):
        ...
        try:
            _ev = table(show.style.format(fmts), hide_index=True, width="stretch",
                               height=340, key=f"tbl_{key}", on_select="rerun",
                               selection_mode="single-row")
        except TypeError:
            _ev = None
            table(show.style.format(fmts), hide_index=True, width="stretch", height=340)
    ...
    if "종합 요약" in page:
        ...
        with _tab_top:
            table(win[_tbcols].rename(columns=_tbren).style.format(_tbfmt),
                         hide_index=True, width="stretch")
        with _tab_bot:
            table(los[_tbcols].rename(columns=_tbren).style.format(_tbfmt),
                         hide_index=True, width="stretch")
        ...
        if len(av) >= 8 and _rsd > 0:
            ...
            def _show_outliers(d, container, emoji, label):
                ...
                with container:                       # 컬럼 안에서도 엑셀 버튼이 같이 붙게
                    table(vv.style.format({"주문CR": "{:.2%}", "발송": "{:,.0f}"}),
                          hide_index=True, width="stretch", dl_name=f"이상치 {label}")
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        if _wk_site_rows:
            st.markdown('<div class="appendix"><b>앱푸시 회원UV는 사이트 전체 지표</b>라 위 발송 '
                        ...
                        f'들어온 유입만 잡은 값이고요. 앱푸시 거래액은 「{menu_ref("회원UV·거래액")}」에서 '
        ...
        with tabP:
            ...
            table(pd.DataFrame(prow).style.map(_clr, subset=["전월비(마감)", "전년비(마감)"]),
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        table(show[["속성", "보유평균", "보유n", "미보유평균", "미보유n", "차이", "효과크기", "유의성(보정)"]],
                     hide_index=True, width="stretch")
        ...
        if len(eff) == 0:
            ...
            table(er[["속성", "순효과", "유의성(보정)"]], hide_index=True, width="stretch", dl_name="다른 조건을 맞춘 뒤 속성의 진짜 효과")
        ...
        # itertools는 모듈 상단에서 import한다 — 여기서 다시 import하면 main() 전체에서
        # itertools가 지역변수로 승격돼, 앞쪽에서 쓰는 곳이 UnboundLocalError로 죽는다.
        st.markdown('<div class="sdiv"></div>', unsafe_allow_html=True)
        ...
        if not crows:
            ...
            table(cshow[_combo_cols].style.format({"캠페인수": "{:,.0f}"}),
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        if cat_rows:
            ...
            if recs:
                ...
                table(pd.DataFrame(recs), hide_index=True, width="stretch", dl_name="카테고리별 추천 소구 (표본 3건 이상만)")
        else:
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        if "dow_k" in base and "hour" in base:
            ...
            if len(slot) == 0:
                ...
                table(disp.sort_values(["_o", "hour"])[["요일", "시간", "캠페인수", "평균", "발송", "거래액"]],
        ...
        if _tags and len(ly_rows):
            ...
            if _trows:
                ...
                st.markdown('<div class="appendix">그날 안 쓴 소구는 막대가 없어요. 건수가 적은 소구는 '
                            ...
                            unsafe_allow_html=True)
                table(_td.pivot_table(index="소구", columns="구분",
                                             values=["건수", "발송", "CTR"], aggfunc="first"),
                             width="stretch", height=min(38 + 35 * len(_tags), 420), dl_name="소구 속성별 CTR — 전년 동요일 vs 기준일")
            else:
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        table(show.style.format({
            "발송": "{:,.0f}", "거래액": "{:,.0f}", "캠페인수": "{:,.0f}",
            "유입UV": "{:,.0f}", "주문건수": "{:,.0f}",
            "유입전환율": "{:.2%}", "주문전환율": "{:.2%}", "RPS": "{:,.0f}"}),
            hide_index=True, width="stretch", height=360, dl_name="주차별 실적")
        ...
        if len(avail) >= 2:
            ...
            table(pd.DataFrame(frows), hide_index=True, width="stretch", dl_name="퍼널 분석 — 어디서 빠질까")
        ...
        if gdt["dt"].nunique() < 2:
            ...
            if len(prev) == 0 or len(recent) == 0:
                ...
                table(pd.DataFrame(cmp_rows), hide_index=True, width="stretch", dl_name="기간 비교 — 최근 vs 직전")
                # 카테고리별 주문전환율 변화 Top
                if "cat" in gdt.columns:
                    ...
                    if cats_common:
                        ...
                        table(ch, hide_index=True, width="stretch", height=260, dl_name="카테고리별 주문전환율 변화 (최근 − 직전)")

        # 피로도 하위탭은 전사 MTD가 있어야 열려서 «다른 그룹»에 모아 뒀다 —
        # 가리키는 이름은 메뉴에서 만든다(번호를 박으면 재정렬 때 썩는다).
        st.markdown('<div class="sdiv"></div>', unsafe_allow_html=True)
        st.caption("🌡️ 인당 발송 건수 기반 피로도 분석은 "
                   f"**「{menu_ref('피로도 시계열').split(' › ')[0]}」**에서 "
                   "볼 수 있어요"
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        if len(bp):
            ...
            st.plotly_chart(fig, width="stretch")
            table(eff_table(bp.sort_values("발송", ascending=False), "BPU"),
                         hide_index=True, width="stretch", dl_name="BPU별 효율")
        ...
        if len(pr):
            ...
            table(eff_table(tshow, "우선순위"), hide_index=True, width="stretch", dl_name="우선순위별 효율")
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        if len(edf) == 0:
            ...
            table(eshow.style.format({"캠페인수": "{:,.0f}"}), hide_index=True, width="stretch", dl_name="이모지별 성과")
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        table(pd.DataFrame(diag), hide_index=True, width="stretch", dl_name="반복 소구 노출에 따른 성과 감쇠(마모) 진단")
        ...
        for t in sel_attrs:
            ...
            _wear_rows.append({
                ...
                "판정": _verdict})
        table(pd.DataFrame(_wear_rows), hide_index=True, width="stretch")
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        if page == "피로도 시계열":
            ...
            with st.expander("📐 CTR 두 가지 — 뭘 봐야 하나", expanded=False):
                st.markdown(
                    "| 지표 | 계산 | 답하는 질문 |\n"
                    "|---|---|---|\n"
                    "| **유입률(발송당)** | 총유입 ÷ 총발송 | 한 건 더 보낼 가치가 있나 |\n"
            ...
            table(pd.DataFrame(rows), hide_index=True, width="stretch", dl_name="추세 분석")
            if not _full:
            ...
            if _need <= set(_dsel.columns) and len(_dsel) >= 20:
                with st.expander("🩺 피로도 진단 — 더 보내면 정말 더 들어올까", expanded=False):
                    ...
                    if len(_lo) >= 5 and len(_hi) >= 5:
                        ...
                        table(pd.DataFrame(_rowsf), hide_index=True, width="stretch")
                    ...
                    if len(_bg) >= 3:
                        ...
                        table(_bg[["_b", "일수", "CTR(고객당)", "지수 ",
                                          "유입률(발송당)", "지수"]]
                                     .rename(columns={"_b": "인당 발송"}),
                                     hide_index=True, width="stretch", dl_name="인당 발송 구간별 — 두 지표가 다르게 읽히는 이유")
        ...
        # ── 요일 패턴 ──
            ...
            if len(dc):
                ...
                table(show, hide_index=True, width="stretch", dl_name="같은 요일에서 적게 보낸 날 vs 많이 보낸 날")
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        if len(d) >= 6:
            ...
            table(qsum.style.format({"캠페인수": "{:,.0f}"}), hide_index=True, width="stretch", dl_name="① CTR vs 주문전환율")
        ...
        _dim = {"카테고리": "cat", "세그먼트": "target",
                "브랜드(문구 자동)": "brand2", "구분": "brand_kind", "영업": "sales_org",
                "브랜드(원본 입력)": "brand"}
        ...
        if dimname.startswith("브랜드(원본"):
            st.caption("원본 입력은 담당자가 손으로 적은 값이라 캠페인명·행사명이 섞여 있어요. "
                       "브랜드끼리 비교하려면 「브랜드(문구 자동)」를 쓰세요.")
        dcol = _dim[dimname]
        ...
        if len(gg) >= 2:
            ...
            table(gs.rename(columns={dcol: dimname})[[dimname, "캠페인수", "주문CR", "AOV", "거래액", "전략"]]
                         .style.format({"캠페인수": "{:,.0f}"}),
                         hide_index=True, width="stretch", dl_name="② 거래액 드라이버 분석: 판매 수량(Q) vs 객")
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        def _rank(dimcol, title, minn, topn):
            ...
            table(gs.rename(columns={dimcol: title})[[title, "캠페인수", "발송", "지표", "거래액"]]
                         .style.format({"캠페인수": "{:,.0f}", "발송": "{:,.0f}"}),
                         hide_index=True, width="stretch", dl_name="발송유형·브랜드 랭킹")
        ...
        _bsrc = st.radio("브랜드 기준", ["문구 자동 분류", "원본 입력"], horizontal=True,
                         key="p15_bsrc",
                         help="원본 입력은 담당자가 손으로 적은 값이라 캠페인명·행사명이 섞여 있어요.")
        _bcol = "brand2" if (_bsrc == "문구 자동 분류" and "brand2" in base) else "brand"
        _rank(_bcol, "브랜드", minn=int(bminn), topn=int(btopn))

        # ── 구분별 성과 — 브랜드 발송 / 전관행사 / 카테고리 묶음이 각각 어떤지 ──
        if "brand_kind" in base:
            st.markdown('<div class="sdiv"></div>', unsafe_allow_html=True)
            st.markdown("##### ③ 구분별 성과 (브랜드 · 전관행사 · 트리거)")
            st.caption("브랜드로 특정되는 발송과 전사 행사·트리거 발송을 갈라서 봐요.")
            _rank("brand_kind", "구분", minn=1, topn=10)

        # ── 자동 분류 점검 — 사전이 실제 문구를 얼마나 덮는지 눈으로 보고 채워 넣는 창구 ──
        if "brand2" in base and "brand_kind" in base:
            st.markdown('<div class="sdiv"></div>', unsafe_allow_html=True)
            with st.expander("🔍 브랜드 자동 분류 점검 — 브랜드로 못 집은 발송"):
                _kv = base["brand_kind"].fillna(KIND_ETC)
                _tot = len(_kv)
                _known = int((_kv == KIND_BRAND).sum())
                st.caption(f"브랜드로 특정된 건 {_known:,}/{_tot:,}건 "
                           f"({_known / max(_tot, 1) * 100:.0f}%) · " +
                           " · ".join(f"{k} {int(v):,}"
                                      for k, v in _kv.value_counts().items() if k != KIND_BRAND))
                # 브랜드로 안 잡힌 것 = 사전에 보탤 후보. 거래액 큰 순으로 본다.
                _un = base[_kv != KIND_BRAND]
                if not len(_un):
                    st.success("전부 브랜드로 분류됐어요.")
                else:
                    _ug = (_un.groupby(_un["brand"].fillna("(빈칸)").replace("", "(빈칸)"))
                           .agg(구분=("brand_kind", "first"), 분류결과=("brand2", "first"),
                                건수=("af", "size"), 발송=("send", "sum"), 거래액=("amt", "sum"),
                                예시문구=("title", lambda s: _s(s.iloc[0])[:40]))
                           .reset_index().rename(columns={"brand": "원본 브랜드칸"})
                           .sort_values("거래액", ascending=False).head(30))
                    table(_ug.style.format({"건수": "{:,.0f}", "발송": "{:,.0f}",
                                                   "거래액": "{:,.0f}"}),
                                 hide_index=True, width="stretch", height=320, dl_name="③ 구분별 성과 (브랜드 · 전관행사 · 트리거)")
                    st.markdown('<div class="appendix">브랜드 사전은 영업별 운영브랜드 시트에서 만든 '
                                '<b>data/brand_map.csv</b>가 정본이에요. 여기 뜬 값 중 실제 브랜드가 '
                                '있는데 안 잡혔다면 시트에 없는 표기(영문·사내 약어)일 가능성이 커요 — '
                                '<b>BRAND_ALIAS</b>에 한 줄 넣으면 바로 반영돼요. 전사 행사면 '
                                '<b>PROMO_KW</b>에 넣으면 전관행사로 묶여요.</div>',
                                unsafe_allow_html=True)
        glossary()
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        if "dow_k" in base and "hour" in base:
            ...
            if len(slot):
                ...
                table(sshow.rename(columns={"dow_k": "요일"})[["요일", "시간", "캠페인수", "지표", "발송"]]
                             .style.format({"캠페인수": "{:,.0f}", "발송": "{:,.0f}"}),
                             hide_index=True, width="stretch", dl_name="추천 발송 슬롯 (요일 × 시간)")
            else:
        ...
        if trows:
            ...
            table(tdf[["소구", "보유평균", "리프트", "보유n", "유의성"]].head(6)
                         .style.format({"보유n": "{:,.0f}"}), hide_index=True, width="stretch", dl_name="추천 소구 및 리프트 성과")
        ...
        if "target" in base and base["target"].fillna("").ne("").any():
            ...
            for sname, sg in bp.groupby("target"):
                ...
                play.append(row)
        if play:
            table(pd.DataFrame(play), hide_index=True, width="stretch", dl_name="세그먼트별 실행 플레이북 (추천 조합)")
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        with tabA:
            ...
            if len(a) == 0:
                ...
                table(promo_perf_table(show.head(15)),
                             hide_index=True, width="stretch", dl_name="기여율 우수 기획전 — 발송 주도형 매출")
                lowbase = show[show["inf_amt"] >= show["inf_amt"].median()]
                st.markdown("**기여율 미흡 기획전 (유입 거래액 중앙값 이상) — 자연 유입 의존도 우세, 마케팅 강화 권장**")
                table(promo_perf_table(lowbase.sort_values("기여율").head(15)),
                             hide_index=True, width="stretch", dl_name="기여율 우수 기획전 — 발송 주도형 매출")
        ...
        with tabB:
            ...
            table(promo_perf_table(b.head(50)),
                         hide_index=True, width="stretch", height=520, dl_name="기획전별 발송 효율 — 발송 대비 실매출")
        ...
        with tabC:
            ...
            table(summary.style.format({"기획전수": "{:,.0f}", "평균매출": "{:,.0f}",
                         "중앙값매출": "{:,.0f}", "합계매출": "{:,.0f}"}),
                         hide_index=True, width="stretch", dl_name="발송한 기획전 vs 발송 안 한 기획전 — 매출 비")
    ...
    # PAGE 07 — 데이터·다운로드
    # ══════════════════════════════════════════════════════════════
    elif "데이터·다운로드" in page:
        st.title("데이터 · 다운로드")
        st.markdown(f"**머지 결과** — 전체 {len(raw)}건 · 문구 매칭 {raw['matched'].sum()}건 "
                    f"({raw['matched'].mean()*100:.0f}%)")
        table(df.drop(columns=["dt"], errors="ignore"), hide_index=True,
                     width="stretch", height=420, dl_name="데이터 · 다운로드")
        ...
        if len(miss):
            ...
            table(miss.rename(columns={"date": "날짜", "af": "AF코드", "cat": "카테고리",
                                              ...
                                              "send": "발송", "amt": "거래액"}),
                         hide_index=True, width="stretch", dl_name="매칭 진단 — 실적은 있지만 문구를 못 찾은 건")
    ...
    # ══════════════════════════════════════════════════════════════
        ...
        if not wk_df.empty:
            ...
            table(wk_show, hide_index=True, width="stretch", height=360, dl_name="주간 지표 (이상치 제외)")
            ...
            def _cross_analysis_block():
                ...
                if len(_joined) >= 8:
                    ...
                    if _ll_rows:
                        ...
                        table(_lt, hide_index=True, width="stretch", dl_name="발송량 → 이탈 시차 탐색 (주간)")
        ...
        with st.expander(f"⚠️ 제외된 이상치 날짜 ({n_outlier}일)"):
            ...
            if not _outlier_rows.empty:
                table(
                    ...
                    hide_index=True, width="stretch", dl_name="발송량 → 이탈 시차 탐색 (주간)")
    ...
    # 남은발송을 양쪽에서 다 뺀 같은 성격끼리의 비교다.
        ...
        st.markdown(f'<div class="vg" style="border-left:4px solid {_pf_col_v}">'
                    ...
                    f'보고, 「{menu_ref("발송량 최적 구간")}」의 인당발송↔효율 회귀와 같이 보면 '
