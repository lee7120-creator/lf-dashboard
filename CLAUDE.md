...
저장소 → 엑셀 → 스모크 → 필터 → 최적구간 → 감축효과 → 사이트지표 → 주간보고** 순으로 자동 실행한다. 두 대시보드나 `tests/`를 건드렸으면 CI가 초록인 걸
보고 머지할 것.
...
- **컨틴 구좌는 회고가 아니라 판단 지원 화면이다.** 2026-08 신설 16시 구좌라 자체 실적이
  ...
  좁혀 보게 했다. 2025년 `컨틴전시 A/B`(주말 17시 308건)는 **성격이 다른 구좌**라 섞어
  평균 내면 안 된다.
...
- 주간보고 MTD 표의 앱·PUSH 행은 **데이터가 없으면 통째로 뺀다**(빈 행이 남으면 0인지 없는
  건지 구분이 안 된다). 값은 누계가 아니라 **그 기간의 일평균**이다 — 발송 실적과 분모가
  달라 합계로 섞을 수 없다.
