...
- **② 퍼널 지표 추이는 ① 바로 뒤에 둔다.** ①은 한 기간의 사진이라 '이번이 낮은 건지
  ...
  **자를 땐 기간을 센다**(`FUNNEL_TREND_KEEP` 일 31·주 16·월 12) — 증감 칸까지 섞어
  `[-N:]`으로 집으면 보이는 기간이 반토막 난다(04에서 실제로 8주로 줄었다).
  ...
  **엑셀은 안 자른다 — 올해 전체를 담는다.** 화면을 자르는 건 눈이 감당 못 해서지 그 뒤가
  필요 없어서가 아니다. 받아서 쓰는 쪽은 연중을 통째로 놓고 보므로 `wtable(dl_data=)`로
  갈라 준다(화면=`tbl[keep]` · 파일=`tbl`). 잘랐으면 캡션에 밝힌다
  (`t_trend_excel_carries_the_whole_year_not_the_screen_slice`).
...
- 아래 「채널 × 기간」 표만 지표 하나를 고른다(자세히 보는 용도).
