...
- **④ 하단 연중 추이 차트는 드릴다운을 따라간다.** 표는 한 기간의 사진이라 '이번이 낮은
  건지 원래 그런 건지'를 못 본다. 지금 보는 대상(전체/조직/카테고리)의 올해 흐름을 전년과
  맞대 **두 줄로만** 그린다 — 항목마다 선을 그리면 카테고리가 많아 안 읽힌다.
  기간 단위(주차/월)는 **위 비교 기준과 별개**로 고른다(주차로 보다 월로 묶는 왕복이 잦다).
  x축은 **올해 라벨**로 세운다 — 전년에만 있는 기간까지 넣으면 축이 늘어져 올해 흐름이
  눌린다. 값이 없는 기간은 `connectgaps=False`로 **선을 끊는다**.
  검사할 땐 **실제로 드릴다운을 밟아야 한다** — 기본(전체)만 보면 `path`를 무시하고 늘
  전체를 그려도 통과한다(실제로 안 잡혔다). 표 클릭은 AppTest가 흉내 못 내니 세션에
  `{"selection": {"cells": [[1, …]]}}`를 심어 리런한다. 두 해 라벨이 같은 합성본으론
  x축 규칙도 안 잡히니 **전년에만 있는 기간을 일부러 심을 것**
  (`t_orgcat_trend_follows_the_drilldown`·`t_orgcat_trend_axis_is_this_year_only`).
