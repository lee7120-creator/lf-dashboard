...
- **② 퍼널 지표 추이는 ① 바로 뒤에 둔다.** ①은 한 기간의 사진이라 '이번이 낮은 건지
  원래 그런 건지'를 못 본다. 표는 03·04와 같은 얼굴(`trend_table(delta_year=)` — 행=지표,
  열=기간, 각 기간 **바로 오른쪽**에 전년 같은 기간 대비 증감)이고, 차트는 한 줄에 세 장이다.
  **자를 땐 기간을 센다**(`FUNNEL_TREND_KEEP` 주 16·월 12) — 증감 칸까지 섞어 `[-N:]`으로
  집으면 보이는 주가 반토막 난다(04에서 실제로 8주로 줄었다).
  지표 위젯은 **multiselect**로 두고 라벨도 '추이에 올릴 지표'로 다르게 쓸 것 — ③·④의
  「지표」 셀렉트를 라벨로 찾는 검사가 있어 같은 이름을 쓰면 엉뚱한 위젯을 집는다.
  블록마다 「기간 단위」 라디오가 따로 있으니 검사는 **키로** 집을 것
  (`wr_fn_ftrend_gran` / `wr_fn_trend_gran`).
